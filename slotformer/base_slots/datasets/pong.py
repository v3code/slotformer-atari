...
class PongDataset(Dataset):
    ...
    def __init__(
        ...
        self.split = split
        self.pong_transform = pong_transform
    ...
    def _read_frames(self, idx):
        ...
        frames = [self.pong_transform(img) for img in frames]
...
class PongSlotsDataset(PongDataset):
...
def build_pong_dataset(params, val_only=False):
    ...
    val_dataset = PongDataset(**args)
    if val_only:
        return val_dataset
    args['split'] = 'train'
    train_dataset = PongDataset(**args)
...
def build_pong_slots_dataset(params, val_only=False):
    ...
    val_dataset = PongSlotsDataset(**args)
    if val_only:
        return val_dataset
    args['split'] = 'train'
    args['video_slots'] = slots['train']
    train_dataset = PongSlotsDataset(**args)
    return train_dataset, val_dataset
